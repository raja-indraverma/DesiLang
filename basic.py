...
class Parser:
	...
	def comp_expr(self):
		...
		if res.error:
			return res.failure(InvalidSyntaxError(
				self.current_tok.pos_start, self.current_tok.pos_end,
				"'nahi', int, float, identifier, '+', '-', ya '(' hona chahiye yaha"
			))
	...
	def expr(self):
		...
		if res.error:
			return res.failure(InvalidSyntaxError(
				self.current_tok.pos_start, self.current_tok.pos_end,
				"'VAR', int, float, identifier, '+', '-' ya '(' hona chahiye yaha"
			))
...
global_symbol_table.set("jhuth", Number(0))
